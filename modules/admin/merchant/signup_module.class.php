...
class signup_module extends api_admin implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {
		...
		if (empty($responsible_person) || empty($email) || empty($mobile) || empty($seller_name) || empty($seller_category) 
		     || empty($validate_type) || empty($province) || empty($city) || empty($address) || empty($longitude) || empty($latitude)) {
		...
		$info_staff_user		= RC_DB::table('store_franchisee')->where('mobile', $mobile)->first();
