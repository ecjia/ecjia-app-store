...
class category_module extends api_front implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {
        ...
        $cat_id = RC_DB::table('store_franchisee')->where('store_id', $seller_id)->selectRaw('cat_id')->first();
        ...
        $cat_list = RC_DB::table('store_category')->selectRaw('parent_id, cat_name, cat_id')->where('cat_id', $cat_id)->get();
        $cat_arr = array();
        foreach($cat_list as $key => $val){
            $cat_arr['id'] = $val['cat_id'];
            $cat_arr['name'] = $val['cat_name'];
            $cat_arr['children'] = get_child_tree($val['cat_id']);
        }
        return $cat_arr;

		// $options = cat_options(0, $cat_arr);
		// $cat_lists = array();
		// if (!empty($options)) {
