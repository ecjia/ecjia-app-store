...
// 		$count_where = array('user_id' => $seller_id, 'is_on_sale' => 1, 'is_alone_sale' => 1, 'is_delete' => 0);
		$count_where = array('seller_id' => $seller_id, 'is_on_sale' => 1, 'is_alone_sale' => 1, 'is_delete' => 0);
...
// 		$relation_row = $db_term_relation->where(array('object_id' => $info['seller_id'], 'object_type' => 'ecjia.merchant', 'object_group' => 'merchant'))->find();
		...
		$favourable_result = $db_favourable->where(array('seller_id' => $info['seller_id'], 'start_time' => array('elt' => RC_Time::gmtime()), 'end_time' => array('egt' => RC_Time::gmtime()), 'act_type' => array('neq' => 0)))->select();
		...
		$seller_info = array(
				...
				'store_banner'  => array(
						'img' 		=> empty($info['item_value1']) ?  '' : RC_Upload::upload_url($info['item_value1']),
						'url'       =>  empty($info['item_value2']) ?  '' : $info['item_value2'],
				),
				'seller_category'	=> $info['cat_name'],
				'shop_name'			=> $info['shop_name'],
				'shop_address'		=> $province_name.' '.$city_name.' '.$info['shop_address'],
				'telephone'			=> $info['kf_tel'],
				'seller_qq'			=> $info['kf_qq'],
