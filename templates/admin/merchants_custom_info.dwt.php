...
<table cellspacing="1" cellpadding="3" width="100%">
  ...
  <tr>
    ...
    <td>
    	<label><input type="radio" name="wintype" value="0" {if $seller_win.win_type eq 0}checked="checked"{/if} onFocus="changecutom(1);" />{t}自定义内容{/t}</label>
        <label><input type="radio" name="wintype" value="1" {if $seller_win.win_type eq 1}checked="checked"{/if} onFocus="changecutom(0);" />{t}商品柜{/t}</label>
   </td>
  </tr>
  <tr>
    ...
    <td>
    	<select name="win_goods_type">
        	<option value="1" {if $seller_win.win_goods_type eq 1} selected="selected"{/if}>{t}样式一{/t}</option>
            <option value="2" {if $seller_win.win_goods_type eq 2} selected="selected"{/if}>{t}样式二{/t}</option>
            <option value="3" {if $seller_win.win_goods_type eq 3} selected="selected"{/if}>{t}样式三{/t}</option>
            <option value="4" {if $seller_win.win_goods_type eq 4} selected="selected"{/if}>{t}样式四{/t}</option>
            <option value="5" {if $seller_win.win_goods_type eq 5} selected="selected"{/if}>{t}样式五{/t}</option>
        </select>
   </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td><input type="radio" name="isshow" value="1" {if $seller_win.is_show eq 1}checked="checked"{/if} /> {$lang.yes}
        ...
        <br/><span class="notice-span">{t}决定是否在店铺首页显示该橱窗{t}</span>
    </td>
  </tr>
  <tr id="cutmedit" style="display:{if $seller_win.win_type eq 1}none{/if}">
    <td class="label">{t}自定义内容{/t}</td>
    <td>{$FCKeditor}</td>
  </tr>
  <tr>
