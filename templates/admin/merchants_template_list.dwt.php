...
<div class="list-div">
  ...
  <div style="display:-moz-inline-stack;display:inline-block;vertical-align:top;zoom:1;*display:inline; padding:10px;">
    <table border="0" cellpadding="0" cellspacing="0" style="width: 220px; text-align:center;">
      <tr>
        <td><strong><a href="{$template.uri}" target="_blank">{$template.name}</a></strong></td>
      </tr>
      <tr>
        <td style="text-align:center">{if $template.screenshot}<img src="{$template.screenshot}" border="0" style="cursor:pointer; float:left; margin:0 2px;display:block;" id="{$template.code}" onclick="javascript:setupTemplate('{$template.code}')"/>{/if}</td>
      </tr>
      <tr>
...
{
  Ajax.call('merchants_template.php?is_ajax=1&act=backup', 'tpl_name=' + tpl, backupTemplateResponse, "GET", "JSON");
}
