				...
				<table class="table table-striped smpl_tbl dataTable table-hide-edit">
   				 ...
   				 <!-- {foreach from=$order_list.item item=order} -->
						<tr>
						    ...
						    <td><i class="{if $order.is_settlement}fontello-icon-ok cursor_pointer{else}fontello-icon-cancel cursor_pointer{/if}" data-trigger="toggleState" data-url='{RC_Uri::url("seller/admin_commission/toggle_state","id={$order.ru_id}&order_sn={$order.order_sn}")}' data-id="{$order.order_id}"></i></td>
						</tr>
						<!-- {foreachelse} -->
